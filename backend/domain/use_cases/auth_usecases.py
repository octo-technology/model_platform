...
def create_access_token(data: dict, expires_delta: Optional[timedelta] = None):
    ...
    token_jwt = jwt.encode(to_encode, Config().jwt_secret, algorithm=Config().jwt_algorithm)
    return token_jwt
...
async def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    access_token = create_access_token(
        {"sub": user.email, "role": user.role}, timedelta(minutes=Config().jwt_access_token_expiration_time)
    )

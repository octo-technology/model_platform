...
from front.api_interactions.projects import add_project, delete_project
...
def create_project_creation_container():
    with st.form("Project Creation"):
        name = st.text_input("Name", placeholder="Insert a project name")
        owner = st.text_input(
            "Owner", placeholder="Insert a project owner (person or organization), responsible for this project."
        )
        scope = st.text_area("Scope", placeholder="Insert a project scope: what is this project about?")
        data_perimeter = st.text_area(
            "Data perimeter", placeholder="Insert a description about the data perimeter of this project."
        )

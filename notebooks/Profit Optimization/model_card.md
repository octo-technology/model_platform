...
OptimModel is a mathematical optimization model designed to maximize profit under given constraints. It uses an integer
linear programming solver to determine the optimal values of two decision variables (x1, x2) while respecting work and
material constraints. The model is implemented using Google's OR-Tools library.
...
- The model does not account for external business or ethical factors, such as sustainability, labor conditions, or
  regulatory constraints beyond the given inputs.
...
**Fairness & Bias**: The model does not use personal or sensitive data; decisions are based solely on numerical
constraints and profit values.

...
class K8SModelDeployment(ModelDeployment):
    ...
    def __init__(self):
        ...
        self.namespace = os.environ["MP_DEPLOYMENT_NAMESPACE"]
    ...
    def delete_model_deployment(self, project_name: str, model_name: str, version: str):
        logger.info(f"Deleting model deployment in {self.namespace} namespace")
        service_name = f"{project_name}-{model_name}-{version}-deployment"
        service_name = sanitize_name(service_name)
        self._delete_model_service_deployment(service_name)
    ...
    def _delete_model_service_deployment(self, service_name: str):
        service_name = sanitize_name(service_name)
        try:
            self.apps_api_instance.read_namespaced_deployment(service_name, self.namespace)
            ...
            logger.info(f"✅ Deployment {service_name} successfully deleted!")
        except ApiException as e:
            if e.status == 404:
                logger.warning(f"⚠️ Deployment {service_name} not found, nothing to delete.")
            else:
                logger.error(f"⚠️ Error while deleting deployment {service_name}: {e}")

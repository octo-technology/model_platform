// Monitoring page — real-time performance metrics for deployed models
const MonitoringPage = (() => {
    ...
    container.innerHTML = `
      <div class="page-header">
        ...
        <div class="page-header-meta">
          <div class="status-badge status-badge-live">
      ...
      <div class="page-content">
        <div class="monitoring-filters mb-6">
          ...
          <div class="filters-row">
            ...
            <div class="form-group"><label class="form-label">Sort</label><select class="form-select" id="monitoring-sort-filter"><option value="name">Name</option><option value="error-rate">Error Rate</option><option value="calls">Calls</option><option value="latency">Latency</option></select></div>
            <div class="form-group"><label class="form-label">Period</label><select class="form-select" id="monitoring-period-filter"><option value="7d">7 Days</option><option value="1d">24 Hours</option><option value="30d">30 Days</option><option value="90d">90 Days</option></select></div>
          </div>
        </div>
        <div id="monitoring-content"><div class="loading-screen"><span class="spinner"></span><span>Loading metrics…</span></div></div>
      </div>
    `;

    loadAllModels();
    ['project', 'status', 'sort', 'period', 'refresh'].forEach(id => {
      document.getElementById(`monitoring-${id}-filter`)?.addEventListener('change', () => filterAndRenderFleet());
      document.getElementById(`monitoring-${id}-btn`)?.addEventListener('click', () => loadAllModels());
    });
  }
  ...
  function filterAndRenderFleet() {
    ...
    const stats = {
      ...
      healthy: filtered.filter(m => getModelStatus(safeMetrics(allMetrics[m.id]).errorRate) === 'healthy').length,
      caution: filtered.filter(m => getModelStatus(safeMetrics(allMetrics[m.id]).errorRate) === 'caution').length,
      alert: filtered.filter(m => getModelStatus(safeMetrics(allMetrics[m.id]).errorRate) === 'alert').length,
      totalCalls: filtered.reduce((s, m) => s + safeMetrics(allMetrics[m.id]).totalCalls, 0)
    };
    ...
    content.innerHTML = `
      <div class="fleet-header">
        ...
        <div><div class="fleet-stat-value">${stats.totalCalls.toLocaleString()}</div><div class="fleet-stat-label">Calls<br><span style="font-size:9px;font-weight:500;opacity:0.7">${PERIOD_LABELS[period]}</span></div></div>
      </div>
      <div class="projects-grid">
        ${Object.entries(grouped).map(([proj, models]) => {
          const pStats = {
            healthy: models.filter(m => getModelStatus(safeMetrics(allMetrics[m.id]).errorRate) === 'healthy').length,
            caution: models.filter(m => getModelStatus(safeMetrics(allMetrics[m.id]).errorRate) === 'caution').length,
            alert: models.filter(m => getModelStatus(safeMetrics(allMetrics[m.id]).errorRate) === 'alert').length
          };
          return `
            <div class="project-box">
              ...
              <div class="models-list">
                ${models.map(m => {
                  ...
                  const suc = Math.round(met.totalCalls * (met.successRate / 100));
                  const errCol = met.totalErrors === 0 ? 'var(--status-green)' : met.totalErrors < 100 ? 'var(--status-orange)' : 'var(--status-red)';
                  return `
                    <div class="model-row">
                      <div class="model-row-header">
                        <div><div style="font-weight:600;font-size:13px">${escHtml(m.name)}</div><div style="font-size:11px;color:var(--text-2)">v${m.version}</div></div>
                      ...
                      <div class="model-row-metrics">
                        ...
                        <div><div class="metric-label">Success</div><div class="metric-value">${suc}</div></div>
                        <div><div class="metric-label">Errors</div><div class="metric-value" style="color:${errCol}">${met.totalErrors}</div></div>

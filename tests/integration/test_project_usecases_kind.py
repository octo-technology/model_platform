...
pytestmark = pytest.mark.skipif(
    not is_kind_running() and not is_kubectl_available(), reason="Kind cluster or kubectl not available"
)
...
class TestProjectMlflowRegistry:
    ...
    def test_mlflow_registry_should_be_accessible(self, login_fixture):
        ...
        # Wait for MLflow to be ready (increased timeout for slow startups)
        deployment_ready = wait_for_deployment_ready(PROJECT_NAME, PROJECT_NAME, timeout=90)
